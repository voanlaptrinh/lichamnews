                    ...
                    @foreach($profile['basic_numbers'] as $type => $data)
                        ...
                        <div class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                            <div class="flex items-center mb-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center">
                                        <span class="bg-{{ $config['color'] }}-500 text-white text-2xl font-bold px-4 py-2 rounded-full mr-3">
                                            {{ $data['number'] }}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">{{ $data['calculation'] }}</p>
                            </div>

                            <div class="mb-4">
                                <h4 class="font-semibold text-gray-700 mb-2">Ý nghĩa:</h4>
                                <p class="text-gray-700 line-clamp-3">{{ Str::limit($data['interpretation'], 150) }}</p>
                            </div>
            ...
            <div id="cycles-content" class="tab-content hidden">
                ...
                <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
                    ...
                    <div class="flex items-center mb-4">
                        ...
                        <div>
                            <p class="text-gray-700 text-lg">{{ $profile['cycles_and_pinnacles']['personal_year']['interpretation'] }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
                    ...
                    <div class="mb-6 text-center">
                        <p class="text-lg text-gray-700">
                            ...
                            (Đang ở <strong class="text-blue-600">Đỉnh cao {{ $pinnacles['current_pinnacle'] }}</strong>)
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        @foreach($pinnacles['pinnacles'] as $i => $pinnacle)
                            ...
                            <div class="text-center p-6 rounded-xl {{ $isActive ? 'bg-gradient-to-br from-green-500 to-green-600 text-white' : 'bg-gray-100' }}">
                                ...
                                <p class="text-xs {{ $isActive ? 'text-green-100' : 'text-gray-600' }}">{{ $pinnacle['phase'] }}</p>
                                @if($isActive)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div class="text-center mb-6">
                            <p class="text-gray-600">Chuỗi ngày sinh: <strong>{{ $birthChart['birth_date_string'] }}</strong></p>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-2 max-w-xs mx-auto mb-6">
                            @foreach($birthChart['chart_grid'] as $row)
                                @foreach($row as $cell)
                                    @php
                                        $freq = $cell['frequency'];
                                        $bgColor = $freq == 0 ? 'bg-red-100 text-red-600' : ($freq >= 3 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-700');
                                    @endphp
                                    <div class="w-20 h-20 border-2 border-gray-300 flex flex-col items-center justify-center {{ $bgColor }}">
                        ...
                        @if(!empty($birthChart['frequency_interpretations']))
                            @php
                                $nonZeroInterpretations = array_filter($birthChart['frequency_interpretations'], function($interpretation) {
                                    ...
                                    <div class="space-y-3 max-h-80 overflow-y-auto">
                                        @foreach($nonZeroInterpretations as $interpretation)
                                            <div class="bg-gray-50 border-l-4 border-blue-400 p-3 rounded">
                                                <h5 class="font-bold text-gray-800 mb-1">{{ $interpretation['interpretation']['summary'] }}</h5>
                                                <p class="text-sm text-gray-700 leading-relaxed">{{ $interpretation['interpretation']['meaning'] }}</p>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="space-y-4">
                            @foreach($arrows['arrows'] as $arrow)
                                @php
                                    ...
                                    $bgColor = $isPresent ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200';
                                ...
                                <div class="p-4 rounded-lg border-2 {{ $bgColor }}">
                                    <div class="flex items-center justify-between mb-2">
                                        <h4 class="font-bold {{ $textColor }}">
                                            {{ $icon }} {{ str_replace(['arrow_of_', 'missing_arrow_of_', '_'], ['Mui tên ', 'Thiếu mui tên ', ' '], $arrow['name']) }}
                                        </h4>
                                        <span class="text-sm {{ $textColor }} font-mono">{{ implode('-', $arrow['numbers']) }}</span>
                                    </div>
                ...
                @if($missingFromName)
                    <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex flex-wrap gap-2 mb-4">
                                    @foreach($missingFromName['missing_numbers'] as $num)
                                        <span class="bg-orange-500 text-white px-3 py-1 rounded-full font-bold">{{ $num }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-wrap gap-2 mb-4">
                                    @foreach($missingFromName['hidden_passions'] as $num)
                                        <span class="bg-green-500 text-white px-3 py-1 rounded-full font-bold">{{ $num }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="bg-orange-50 p-4 rounded-lg">
                            <p class="text-sm text-orange-800">{{ $missingFromName['interpretation'] ?? 'Phân tích các số thiếu hụt và tiềm ẩn từ chữ cái trong tên bạn.' }}</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex flex-wrap gap-2 mb-4">
                                    @foreach($karmicLessons['missing_numbers'] as $num)
                                        <span class="bg-yellow-500 text-white px-3 py-1 rounded-full font-bold">{{ $num }}</span>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                @foreach($karmicLessons['karmic_lessons'] as $lesson)
                                    <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                                        ...
                                        <p class="text-yellow-700 text-sm leading-relaxed">{{ $lesson['meaning'] }}</p>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <div class="text-center py-8">
                                ...
                                <p class="text-gray-600 text-sm">Điều này cho thấy bạn đã có đầy đủ kinh nghiệm từ các kiếp trước.</p>
                            </div>
                        @endif
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        @else
                            <div class="text-center py-8">
                                <div class="text-6xl mb-4">✨</div>
                                <p class="text-green-600 font-semibold text-lg">{{ $karmicDebt['interpretation'] }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div class="mb-6 p-4 bg-blue-50 rounded-lg">
                            <p class="text-gray-700 text-sm leading-relaxed">
                                4 năng lực bẩm sinh được tính dựa trên các chữ cái trong tên của bạn, phản ánh những khả năng tự nhiên mà bạn sở hữu từ khi sinh ra.
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach(['physical', 'emotional', 'intellectual', 'spiritual'] as $type)
                                @if(isset($abilities[$type]))
                                    ...
                                    @endphp
                                    <div class="p-6 border-2 border-{{ $abilityColors[$type] }}-100 rounded-xl bg-{{ $abilityColors[$type] }}-50">
                                        <div class="flex items-center mb-4">
                                            <span class="text-3xl mr-3">{{ $abilityIcons[$type] }}</span>
                                            <div class="flex-1">
                                                <h4 class="text-lg font-bold text-{{ $abilityColors[$type] }}-800">{{ $abilityNames[$type] }}</h4>
                                                <p class="text-{{ $abilityColors[$type] }}-600 font-semibold">{{ $data['title'] }}</p>
                                                <p class="text-sm text-{{ $abilityColors[$type] }}-500">{{ $data['count'] }} chữ cái: {{ implode(', ', $data['letters']) }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            <div class="bg-gray-200 rounded-full h-3">
                                                <div
                                                    class="bg-{{ $abilityColors[$type] }}-500 h-3 rounded-full transition-all duration-500"
                                                    style="width: {{ $percentage }}%"
                                                ></div>
                                            </div>
                                            <p class="text-xs text-{{ $abilityColors[$type] }}-600 mt-1">Mức độ: {{ $data['level'] }}</p>
                                        </div>

                                        <p class="text-{{ $abilityColors[$type] }}-700 text-sm leading-relaxed">{{ $data['description'] }}</p>
                                    </div>
                                @endif
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                            @foreach($luckyColors['colors'] as $color)
                                <div class="text-center">
                                    <div
                                        class="w-20 h-20 mx-auto mb-2 border-4 border-gray-300 rounded-full"
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-white rounded-xl shadow-lg p-8">
                            ...
                            <div class="flex justify-center gap-4 mb-6">
                                @foreach($luckyNumbers['numbers'] as $number)
                                    <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 text-white rounded-full flex items-center justify-center text-2xl font-bold">
                                        {{ $number }}
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="bg-white rounded-xl shadow-lg p-8">
                            ...
                            <div class="text-center">
                                <h4 class="text-xl font-bold text-gray-800 mb-4">{{ $dailyColor['day_of_week'] }}</h4>
                                <div
                                    class="w-24 h-24 mx-auto mb-4 border-4 border-gray-300 rounded-full"
                                    style="background-color: {{ $dailyColor['color']['hex'] }}"
                                ></div>
                                <h5 class="text-lg font-bold text-gray-800">{{ $dailyColor['color']['name'] }}</h5>
                                <p class="text-sm text-gray-600 mb-2">{{ $dailyColor['color']['hex'] }} • Hành {{ $dailyColor['color']['hanh'] }}</p>
                                <p class="text-sm text-gray-700 bg-gray-50 p-3 rounded">{{ $dailyColor['interpretation'] }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-12 text-center">
            <div class="bg-white rounded-xl shadow-lg p-6 inline-block">
                ...
                <div class="flex gap-4">
                    <button onclick="window.print()" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg transition">
                        🖨️ In kết quả
                    </button>
                    <a href="{{ route('numerology.index') }}" class="bg-purple-500 hover:bg-purple-600 text-white px-6 py-3 rounded-lg transition">
                        🔄 Tính toán lại
                    </a>
                    <button onclick="shareResults()" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg transition">
                        📤 Chia sẻ
                    </button>
                </div>
            </div>
