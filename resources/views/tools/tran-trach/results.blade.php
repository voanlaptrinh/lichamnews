                        ...
                        @if (isset($birthdateInfo))
                            <div class="info-grid">
                                ...
                                <p class="mb-2">
                                    ...
                                    {{ $year - explode('/', $birthdateInfo['dob']->format('d/m/Y'))[2] + 1 }} tuổi
                                </p>
                                <p class="mb-2">
                                    <strong>Thời gian trấn trạch:</strong>
                                    {{ $inputs['date_range'] ?? '' }}
                                </p>
                            </div>
                        @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(() => {
            if (typeof initTabooFilter === 'function') {
                console.log('Tran Trach: Initializing taboo filter with data:', resultsByYear);
                ...
                Object.keys(resultsByYear || {}).forEach(year => {
                    if (resultsByYear[year] && resultsByYear[year].days && resultsByYear[year].days[0]) {
                        console.log(`Tran Trach Year ${year} first day structure:`, resultsByYear[year].days[0]);
                        console.log(`Tran Trach Year ${year} day_score structure:`, resultsByYear[year].days[0].day_score);
                        console.log(`Tran Trach Year ${year} checkTabooDays:`, resultsByYear[year].days[0].day_score?.checkTabooDays);
                    }
                });
