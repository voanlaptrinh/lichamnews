 <div class="col-xl-3  col-sm-12 col-12 mb-3">
     <div class="d-flex flex-column gap-4 box-siderbar-index mb-3">
         <!-- ** KHá»I Sá»° KIá»†N Sáº®P Tá»šI ** -->


         <div class="events-card">
             <div class="card-title-right title-tong-quan-h5-log content-title-home-lich-right">Tiện ích xem hướng</div>
             <ul class="list-group list-group-flush events-list">
                 <li class="list-group-item pb-0">
                     <a href="{{ route('huong-nha.form') }}" class="{{ request()->routeIs('huong-nha.form') ? 'active-tools' : '' }}">
                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng nhà
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('huong-ban-tho.form') }}" class="{{ request()->routeIs('huong-ban-tho.form') ? 'active-tools' : '' }}">
                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng bàn thờ
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('huong-bep.form') }}" class="{{ request()->routeIs('huong-bep.form') ? 'active-tools' : '' }}">
                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng bếp nấu
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('huong-phong-ngu.form') }}" class="{{ request()->routeIs('huong-phong-ngu.form') ? 'active-tools' : '' }}">
                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng phòng ngủ
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('huong-ban-lam-viec.form') }}" class="{{ request()->routeIs('huong-ban-lam-viec.form') ? 'active-tools' : '' }}">
                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng bàn làm việc
                             </div>

                         </div>
                     </a>
                 </li>
             </ul>
         </div>


     </div>
     <div class="d-flex flex-column gap-4 box-siderbar-index">
         <!-- ** KHá»I Sá»° KIá»†N Sáº®P Tá»šI ** -->


         <div class="events-card">
             <div class="card-title-right title-tong-quan-h5-log content-title-home-lich-right">Tiện ích Khác</div>
             <ul class="list-group list-group-flush events-list">
                 <li class="list-group-item pb-0">
                     <a href="{{ route('convert.am.to.duong') }}" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Đổi ngày âm dương
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('totxau.list') }}" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem ngày tốt
                             </div>

                         </div>
                     </a>
                 </li>

                 <li class="list-group-item pb-0">
                     <a href="{{ route('horoscope.index') }}" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Cung hoàng đạo
                             </div>

                         </div>
                     </a>
                 </li>
                 {{-- <li class="list-group-item pb-0">
                     <a href="" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Văn khấn
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Thần số học
                             </div>

                         </div>
                     </a>
                 </li> --}}
                 <li class="list-group-item pb-0">
                     <a href="{{ route('laso.create') }}" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Lá số tử vi
                             </div>

                         </div>
                     </a>
                 </li>
                 <li class="list-group-item pb-0">
                     <a href="{{ route('hoptuoi.list') }}" class="">

                         <div class="event-details --padding-event-tot">
                             <div class="event-name" style="font-weight: unset">
                                 Xem hướng hợp tuổi
                             </div>

                         </div>
                     </a>
                 </li>
             </ul>
         </div>


     </div>
 </div>
