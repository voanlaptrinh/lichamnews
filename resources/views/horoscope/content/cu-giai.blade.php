@extends('welcome')

@section('content')
    <div class="container-setup">
          <nav aria-label="breadcrumb" class="content-title-detail">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="{{ route('home') }}" style="color: #2254AB; text-decoration: underline;">Trang chủ</a>
                </li>
                <li class="breadcrumb-item" aria-current="page">
                    <a href="{{ route('horoscope.index') }}" style="color: #2254AB; text-decoration: underline;">Cung
                        Hoàng Đạo</a>
                </li>

                <li class="breadcrumb-item active" aria-current="page">
                    Cự Giải
                </li>
            </ol>
        </nav>
       
        <h1 class="content-title-home-lich">Giới Thiệu Cung Cự Giải</h1>
        <div class="row mt-3">
            <div class="col-lg-9">
                <div class="tong-quan-date mt-2 mb-3">
                    <div class="card-body  p-lg-4 p-3 position-relative" style="border-radius: 24px">
                        <div class="text-box-tong-quan">
                            <h2 class="title-tong-quan-h4">Cung Cự Giải là gì?</h2>
                            <p class="mb-0">Cung Cự Giải (Cancer) là cung hoàng đạo thứ tư, đại diện cho cảm xúc, trực
                                giác và sự
                                che chở.
                                <br>
                                Những người thuộc cung này sinh từ 21/6 đến 22/7, mang biểu tượng là Con Cua (Crab) – loài
                                sinh vật
                                sống giữa đất và nước, tượng trưng cho tính cách nhạy cảm, dễ xúc động nhưng cũng rất mạnh
                                mẽ khi
                                cần bảo vệ bản thân và người thân yêu.

                            </p>
                            <p class="mb-0">
                                Trong chiêm tinh học phương Tây, Cự Giải thuộc nhóm Nước (Water) – nhóm cung cảm xúc và sâu
                                sắc
                                nhất, cùng với Bọ Cạp và Song Ngư.

                                <br>
                                Hành tinh chủ quản của cung này là Mặt Trăng (Moon) – biểu tượng của cảm xúc, trực giác và
                                bản năng
                                làm mẹ, khiến Cự Giải luôn là người giàu lòng trắc ẩn, dịu dàng và đầy yêu thương.
                            </p>
                            <h2 class="title-tong-quan-h4 pt-2">Biểu tượng và chòm sao gắn liền</h2>
                            <ul class="mb-0">
                                <li>Biểu tượng: Con Cua – biểu trưng cho sự bảo vệ và lòng trung thành.

                                </li>
                                <li>Nguyên tố: Nước</li>
                                <li>Hành tinh chủ quản: Mặt Trăng</li>
                                <li>Màu sắc may mắn: Trắng, bạc, xanh lam nhạt</li>
                                <li>Đá phong thủy hợp mệnh: Ngọc trai, đá mặt trăng (Moonstone), thạch anh trắng</li>
                                <li>Con số may mắn: 2, 7, 11</li>
                                <li>Ngày sinh: Từ 21/6 đến 22/7</li>
                            </ul>
                            <p class="mb-0">Chòm sao Cancer gắn liền với truyền thuyết về con cua khổng lồ mà nữ thần Hera
                                gửi đến
                                để bảo vệ bạn của mình trong trận chiến với anh hùng Hercules. Dù bị đánh bại, lòng trung
                                thành của
                                con cua được vinh danh trên bầu trời như một chòm sao bất tử – biểu trưng cho sự tận tâm và
                                hi sinh.
                            </p>

                            <h2 class="title-tong-quan-h4 pt-2">Tính cách đặc trưng của Cự Giải</h2>
                            <p class="mb-0">Cự Giải là người sống nội tâm, tình cảm và luôn đặt trái tim lên hàng đầu. Họ
                                yêu
                                thương, biết quan tâm và sẵn sàng hi sinh để bảo vệ những người mình yêu quý.
                                <br> Bề ngoài có thể trầm lặng, nhưng bên trong là một tâm hồn phong phú và nhạy cảm.
                            </p>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Ưu điểm:</h3>
                            <ul class="mb-0">
                                <li>Rất tình cảm, nhân hậu, luôn biết lắng nghe và thấu hiểu.
                                </li>
                                <li>Giàu trực giác, cảm nhận được cảm xúc của người khác.</li>
                                <li>Trung thành, gắn bó, đáng tin cậy trong mọi mối quan hệ.</li>
                                <li>Sống có trách nhiệm, coi trọng gia đình và các giá trị truyền thống.</li>
                            </ul>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Nhược điểm:</h3>
                            <ul class="mb-1">
                                <li>Hay suy nghĩ, dễ tổn thương và hay lo xa.</li>
                                <li>Có xu hướng “ẩn mình” khi buồn hoặc bị tổn thương.
                                </li>
                                <li>Đôi khi hơi kiểm soát và bảo vệ quá mức người thân.</li>
                                <li>Khó buông bỏ quá khứ, dễ vương vấn trong chuyện cũ.</li>
                            </ul>
                            <p class="mb-0">Cự Giải sống theo cảm xúc, nhưng chính sự nhạy cảm này giúp họ trở thành những
                                người bạn tuyệt vời, người yêu chân thành và thành viên gia đình mẫu mực.

                            </p>
                            <h2 class="title-tong-quan-h4 pt-2">Sự nghiệp và nghề nghiệp phù hợp</h2>
                            <p class="mb-1">Người thuộc cung Cự Giải có trực giác tốt, khả năng quan sát tinh tế và tính
                                cách chu đáo. <br>
                                Họ thích những công việc mang tính ổn định, có ý nghĩa hoặc giúp đỡ người khác.

                            </p>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Nghề nghiệp lý tưởng cho Cự Giải:</h3>
                            <ul class="mb-1">
                                <li>Bác sĩ, y tá, tư vấn tâm lý, giáo viên
                                </li>
                                <li>Đầu bếp, nhà thiết kế nội thất, nghệ sĩ</li>
                                <li>Nhà văn, nhiếp ảnh gia, nhạc sĩ
                                </li>
                                <li>Nhân viên chăm sóc khách hàng, quản lý nhân sự</li>
                                <li>Các công việc liên quan đến gia đình, giáo dục hoặc cộng đồng</li>
                            </ul>
                            <p class="mb-0">Cự Giải không thích môi trường quá cạnh tranh hoặc khô khan. Họ phát triển tốt
                                nhất trong không gian ấm áp, thân thiện và có tính nhân văn.


                            </p>
                            <h2 class="title-tong-quan-h4 pt-2">Tình yêu và các mối quan hệ</h2>
                            <p class="mb-0">Trong tình yêu, Cự Giải là người chân thành, sâu sắc và cực kỳ chung thủy.
                                <br>
                                Họ luôn muốn xây dựng một mối quan hệ lâu dài, ổn định và tràn đầy yêu thương. <br>
                                Một khi đã yêu, họ dành trọn tâm trí để chăm sóc, bảo vệ và vun vén cho đối phương.

                            </p>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Đặc điểm tình cảm:</h3>
                            <ul class="mb-0">
                                <li>Rất quan tâm và luôn đặt cảm xúc người yêu lên trước bản thân.

                                </li>
                                <li>Lãng mạn, tinh tế và dễ xúc động.
                                </li>
                                <li>Dễ bị tổn thương nếu không được đáp lại tình cảm như mong đợi.</li>
                                <li>Cần cảm giác an toàn và được yêu thương thật lòng.
                                </li>

                            </ul>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Hợp với các cung:</h3>
                            <ul class="mb-0">
                                <li>Thiên Yết (Bọ Cạp/Bọ Cạp): cùng nhóm Nước, đồng điệu cảm xúc mạnh mẽ.

                                </li>
                                <li>Song Ngư: cùng mơ mộng, dễ thấu hiểu và chia sẻ.
                                </li>
                                <li>Kim Ngưu: vững vàng, tạo cảm giác an toàn cho Cự Giải.</li>
                            </ul>
                            <h3 class="title-tong-quan-h5 pt-2 mb-0">Không hợp lắm với:</h3>
                            <ul class="mb-0">
                                <li>Bạch Dương: quá mạnh mẽ, dễ khiến Cự Giải tổn thương.
                                </li>
                                <li>Nhân Mã: thích tự do, khó đem lại cảm giác ổn định mà Cự Giải cần.
                                </li>
                            </ul>
                            <h2 class="title-tong-quan-h4 pt-2">Cự Giải trong cuộc sống</h2>
                            <p class="mb-1">
                                Cự Giải sống thiên về tình cảm, yêu thích sự yên bình và luôn hướng về gia đình. <br>
                                Họ có xu hướng lưu giữ kỷ niệm, yêu thích những điều nhỏ bé, giản dị nhưng ý nghĩa. <br>
                                Là người chu đáo và tinh tế, Cự Giải thường được bạn bè tin tưởng và tìm đến khi cần lời
                                khuyên chân thành.

                            </p>
                            <p class="mb-0">Tuy nhiên, họ cần học cách mở lòng và kiểm soát cảm xúc, tránh để quá khứ hoặc
                                nỗi sợ tổn thương ảnh hưởng đến hiện tại.
                            </p>
                            <h2 class="title-tong-quan-h4 pt-2">Tổng kết</h2>
                            <p class="mb-0">Cung Cự Giải (Cancer) là biểu tượng của tình yêu thương, cảm xúc và gia đình.
                                Họ là những người sống sâu sắc, biết quan tâm và luôn hướng tới việc xây dựng mối quan hệ
                                bền vững, đầy ấm áp.
                            </p>
                            <p class="mb-0">Nếu bạn là Cự Giải – hãy tự hào vì bạn chính là trái tim ấm áp của 12 cung
                                hoàng đạo, mang trong mình sức mạnh của yêu thương, lòng bao dung và sự kiên định trong cảm
                                xúc.
                            </p>
                        </div>
                    </div>

                </div>
                  @include('horoscope.list-cung')
            </div>
              @include('horoscope.box-right')
        </div>
          
    </div>
@endsection
